...
(comment
(map #(.addPost delicious (url %) (title %) "Google notebook export" (str-join " " (labels %)) (java.util.Date.) false false) (notes dz) )

(let [n (first (notes dz))]
;  (.addPost delicious (url n) (title n) "Google notebook export" (str-join " " (conj "google-notebook-import" (labels n))) (java.util.Date.) false false)
  (println  (url n) (title n) "Google notebook export" (str-join " " (conj "google-notebook-import" (labels n))) )
)

(loop [note (first-note(container dz)) acc [] ]
  (println (note-url note))
  (if (nil? note)
    acc
    (recur (next-note note) (conj acc (note-url note)))
  )
)
...
(loop [i 1 sum 0]
 ...
 (ns com.raphinou.nb_exporter)
 (def dz (zip/xml-zip (xml/parse  (user-input "enter html file result of google notebook export between double quotes:")  startparse-tagsoup) ))
